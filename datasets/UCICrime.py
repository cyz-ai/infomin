...
def read_dataset(name, label=None, sensitive_attribute=None, fold=None):
    ...
    if name=='adult':
        return load_adult(sensitive_attribute)
    else:
...
def load_adult(sensitive_attribute='race', nTrain=None, scaler=True, shuffle=False):
    if shuffle:
    ...
    if sensitive_attribute == 'race':
        z = data['race'].values
        features = data.drop(['race'], axis=1).values
    else:
        z = data['Age'].values
        features = data.drop(['Age'], axis=1).values
        
    
    #Care there is a final dot in the class only in test set which creates 4 different classes
    y = np.array([-1.0 if (val == 0 or val==1) else 1.0 for val in np.array(features)[:, -1]])
